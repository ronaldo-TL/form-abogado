    ...
    <form @submit.prevent="actualizarAbogado" class="mt-5">
      ...
      <div class="input-field">
        <select v-model="estadoRegistro" id="estadoRegistro" class="square">
          ...
          <option value="matricula_vigente">Matrícula Vigente</option>
          <option value="matricula_no_vigente">Matrícula No Vigente</option>
        </select>
...
export default {
  ...
  methods: {
    async actualizarAbogado() {
      try {
        const respuesta = await axios.get(`http://192.168.3.243/apirpaoficial/web/publico/abogado/6063892SCG/actualizar-campo`, {
          params: {
